...
namespace YHYXYH.YXYH
{
    ...
    { 
        ...
        DataTable dtControls = SQLHelper.ExecuteDt("select * from [ControlConfig] where Parent like 'YXYHMain3%' order by Parent asc");
        DataView viewTagLabelBinding = TableTagLabelBinding.getDataTableByRowFilter("[Parent] like 'YXYHMain3%'").DefaultView;
        ...
        public void SetLabelVisible(bool bHCGDChecked)
        {
            ...
            dtControls = TableControlConfig.getDataTableByRowFilter("[Parent] like 'YXYHMain3.%' and [TagID] in (" + sUnit1HCGD_IDs + "," + sUnit1HCGR_IDs + ")");
            foreach (DataRow row in dtControls.Rows)
            {
                try
                {
