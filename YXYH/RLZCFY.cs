...
{
    public partial class RLZCFY : Form
    {
        ////这里注释掉的都要看一遍 ZZH 2016-9-17 
        int UnitNO = 5;
        string F10, F3225;
        public RLZCFY(int unitNo)
        {
            InitializeComponent();
            this.UnitNO = unitNo;
            if (UnitNO == 5)
        ...
        private void RLZCFY_Load(object sender, EventArgs e)
        {
            ...
            chart1.Series[0].YValueMembers = F10;
        ...
        private void timerChartBind_Tick(object sender, EventArgs e)
        {
            ...
            if (lTotalCount < TagLJValue.GetTotalCount(UnitNO))
            {
                ...
                if (dtChart.Rows.Count > 0)
                {
                    ...
                    chart1.ChartAreas[0].AxisY.Title = "燃料累计支出：" + Math.Round(double.Parse(dtChart.Rows[dtChart.Rows.Count - 1][F10].ToString()), 2) + "（万元）";
                    row = dtChart.NewRow();
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sSql = "insert into QueryAlarm(QueryDate,StartTime,EndTime,QueryFields,UnitNO,AlarmDesc) values('"
                    + sTime + "','" + dateTimePickerBegin2.Value + "','" + dateTimePickerEnd2.Value + "','" + F10 + "',"
                    + UnitNO + ",'查询数据时，未按提示等待，做其他操作造成程序退出！')";
                try { SQLHelper.ExecuteSql(sSql); }
                catch { }

                dtChart = TagLJValue.QueryDBTotalData(dateTimePickerBegin2.Value, dateTimePickerEnd2.Value, "[ValueTime],[HourOfYear]," + F10, UnitNO);
                if (dtChart.Rows.Count > 0)
                {
                    if (rdoTotalHours.Checked == true)
                    {
        ...
        private void RLZCFY_FormClosed(object sender, FormClosedEventArgs e)
        {
            dtChart.Dispose();
            dtChart = null;
            this.Dispose(true);
        ...
        {
            DataTable dt = dtChart.DefaultView.ToTable(false, "ValueTime", F10, "HourOfYear");
            string title = chart1.ChartAreas[0].AxisX.Title;
            dt.Columns[0].ColumnName = "时间";
            dt.Columns[1].ColumnName = "燃料累计支出";
            dt.Columns[2].ColumnName = "小时数";
            bool b = NPOIExcelHelper.DataTableToExcel(dt, title, "燃料累计支出");
            if (b)
