...
{
    public partial class GRLJSR : Form
    {
        ////这里注释掉的都要看一遍 ZZH 2016-9-17 
        int UnitNO = 5;
        string F2, F3227;
        public GRLJSR(int unitNo)
        {
            InitializeComponent();
            this.UnitNO = unitNo;
            if (UnitNO == 5)
        ...
        private void GRLJSR_Load(object sender, EventArgs e)
        {
            ...
            chart1.Series[0].YValueMembers = F2;
        ...
        private void timerChartBind_Tick(object sender, EventArgs e)
        {
            ...
            if (lTotalCount < TagLJValue.GetTotalCount(UnitNO))
            {
                ...
                if (dtChart.Rows.Count > 0)
                {
                    ...
                    chart1.ChartAreas[0].AxisY.Title = "供热累计收入：" + Math.Round(double.Parse(dtChart.Rows[dtChart.Rows.Count - 1][F2].ToString()), 2) + "（万元）";
                    row = dtChart.NewRow();
            ...
            MinMaxValue mmv = TagLJValue.GetMinMaxValue(F3227, UnitNO);
            chart1.ChartAreas[0].AxisX.Title = "供热期累计小时数：" + Math.Round(dHourOfYear, 4).ToString("0.0000")
                + "                累计开始时间：" + mmv.BeginDateString + "\n供热收入最小值："
                + Math.Round(mmv.MinValue / 10000, 4).ToString("0.0000") + "        供热收入最小值时间：" + mmv.MinDateString
                + "\n供热收入最大值：" + Math.Round(mmv.MaxValue / 10000, 4).ToString("0.0000") + "        供热收入最大值时间："
                + mmv.MaxDateString + "\n供热收入平均值：" + TagLJValue.GetTotalDataAvg(F2, UnitNO) + "万元/小时";
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sSql = "insert into QueryAlarm(QueryDate,StartTime,EndTime,QueryFields,UnitNO,AlarmDesc) values('"
                    + sTime + "','" + dateTimePickerBegin2.Value + "','" + dateTimePickerEnd2.Value + "','" + F2 + "',"
                    + UnitNO + ",'查询数据时，未按提示等待，做其他操作造成程序退出！')";
                try { SQLHelper.ExecuteSql(sSql); }
                catch { }

                dtChart = TagLJValue.QueryDBTotalData(dateTimePickerBegin2.Value, dateTimePickerEnd2.Value, "[ValueTime],[HourOfYear]," + F2, UnitNO);
                if (dtChart.Rows.Count > 0)
                {
                    if (rdoTotalHours.Checked == true)
                    {
        ...
        private void GRLJSR_FormClosed(object sender, FormClosedEventArgs e)
        {
            dtChart.Dispose();
            dtChart = null;
            this.Dispose(true);
        ...
        {
            DataTable dt = dtChart.DefaultView.ToTable(false, "ValueTime", F2, "HourOfYear");
            string title = chart1.ChartAreas[0].AxisX.Title;
            dt.Columns[0].ColumnName = "时间";
            dt.Columns[1].ColumnName = "供热累计收入";
            dt.Columns[2].ColumnName = "小时数";
            bool b = NPOIExcelHelper.DataTableToExcel(dt, title, "供热累计收入");
            if (b)
