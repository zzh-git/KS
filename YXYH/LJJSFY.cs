...
{
    public partial class LJJSFY : Form
    {
        ...
        private void LJMLR_Load(object sender, EventArgs e)
        {
            ...
            chart1.Series[0].YValueMembers = F19;
        ...
        private void timerChartBind_Tick(object sender, EventArgs e)
        {
            ...
            if (lTotalCount < TagLJValue.GetTotalCount(UnitNO))
            {
                ...
                if (dtChart.Rows.Count > 0)
                {
                    ...
                    chart1.ChartAreas[0].AxisY.Title = "累计节省费用：" + Math.Round(double.Parse(dtChart.Rows[dtChart.Rows.Count - 1][F19].ToString()), 2) + "万元";
                    row = dtChart.NewRow();
            ...
            if (UnitNO == 5)
                chart1.ChartAreas[0].AxisX.Title = "供热期累计小时数：" + Math.Round(dHourOfYear, 4).ToString("0.0000")
                    ...
                    + (mmv.MinValue * dF3198).ToString("0.0000") + "        实时节省费用最小值时间：" + mmv.MinDateString
                    + "\n实时节省费用最大值：" + (mmv.MaxValue * dF3198).ToString("0.0000") + "        实时节省费用最大值时间："
                    + mmv.MaxDateString + "\n实时节省费用平均值：" + TagLJValue.GetTotalDataAvg(F19, UnitNO) + "元/小时        累计节省费用："
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sSql = "insert into QueryAlarm(QueryDate,StartTime,EndTime,QueryFields,UnitNO,AlarmDesc) values('"
                    + sTime + "','" + dateTimePickerBegin2.Value + "','" + dateTimePickerEnd2.Value + "','" + F19 + "',"
                    + UnitNO + ",'查询数据时，未按提示等待，做其他操作造成程序退出！')";
                try { SQLHelper.ExecuteSql(sSql); }
                catch { }

                dtChart = TagLJValue.QueryDBTotalData(dateTimePickerBegin2.Value, dateTimePickerEnd2.Value, "[ValueTime],[HourOfYear]," + F19, UnitNO);
                if (dtChart.Rows.Count > 0)
                {
                    if (rdoTotalHours.Checked == true)
                    {
        ...
        {
            DataTable dt = dtChart.DefaultView.ToTable(false, "ValueTime", F19, "HourOfYear");
            string title = chart1.ChartAreas[0].AxisX.Title;
            dt.Columns[0].ColumnName = "时间";
            dt.Columns[1].ColumnName = "累计节省费用";
            dt.Columns[2].ColumnName = "小时数";
            bool b = NPOIExcelHelper.DataTableToExcel(dt, title, "累计节省费用");
            if (b)
