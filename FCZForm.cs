...
namespace KSPrj
{
    public partial class FCZForm : Form
    {
        ...
        public FCZForm()
        {
            ...
            LabelDYGPQWD = this.panelChild.Controls.Find("LabelDYGPQWD", false)[0] as Label;
            LabelZYGPQWD = this.panelChild.Controls.Find("LabelZYGPQWD", false)[0] as Label;
            ...
            LabelBindDoubleClick(this.label43, 3416, "凝汽器真空（kPa）");
            LabelBindDoubleClick(this.labelSub1, 3002, "末级叶片颤振值");
        ...
        private void init()
        {
            ...
            chart1.Series[0].LegendText = "发电煤耗";
        ...
        private void ChangeLabels(DataTable dt)
        {
            ...
            {
                double fuhe = 0;
                fuhe = TagValue.GetFinishedTagValueFive("F1145");
                //负荷小于100  就不做报警判断了
            ...
            {
                //原始数据若有不合法，开始报警
                if (!CheckTags(GlobalVariables.dtOneRowDataFive))
                    btnTagBool = true;
                else
                {
                    btnTagBool = false;
        ...
        private void SetControlMouseEvent()
        {
            ...
            {
                control = this.groupBox2.Controls[i];
                if (control.Tag != null)
                {
                    if (GlobalVariables.IsCanMoveLabel)
                    {
            ...
            for (int i = 0; i < this.panelChild.Controls.Count; i++)
            {
                control = this.panelChild.Controls[i];
                if (control.Tag != null)
                {
                    if (GlobalVariables.IsCanMoveLabel)
                    {
        ...
        private void PicChartBind()
        {
            try
            {
                double tdouBar1Value = TagValue.GetFinishedTagValueFive("F3002");
                ...
                if (douBar1Value <= 100)
                {
                    ...
                    if (douBar1Value < 1)//如果低于10，则按照10的长度显示
                    {
                        this.picGreen1.Width = Convert.ToInt32(1 * 4 * panel10Scalex);
                    }
                    else
                    {
        ...
        void AddConfigedControl()
        {
            try
            {
                DataTable dtControls = TableControlConfig.getDataTableByRowFilter("[Parent] like 'FCZForm.%'");
                //int i;
                Control parent = null;
        ...
        void SetBindingLabelsText()
        {
            try
            {
                ...
                LabelTag labelTag = null;
                DataTable dtTagLabelBinding = TableTagLabelBinding.getDataTableByRowFilter("[Parent] like 'FCZForm.%'");
        ...
        {
            //SelectTagDate selectTagDate = new SelectTagDate(5, "FCZ");
            ...
            SelectTagDate selectTagDate = new SelectTagDate(5,chart1, "FCZ");
            selectTagDate.Show();
        }
        ...
        {
            KSPrj.Tool.AlarmList aList = new KSPrj.Tool.AlarmList("FCZ", 5);
            aList.Show();
        }
        ...
        {
            TagReplace tr = new TagReplace(5);
            tr.Show();
        }
        ...
        private void dataBindChart()
        {
            ...
            chart1.DataSource = GlobalVariables.dtChartDataFive;
            chart1.DataBind();
            //chart1.Refresh();
        }
        ...
        {
            if (GlobalVariables.dtChartDataFive.Rows.Count > 0)
            {
                this.chart1.DataSource = GlobalVariables.dtChartDataFive;
        ...
        {
            double xValue = TagValue.GetFinishedTagValueFive("F1329");
            if (xValue == GlobalVariables.BadValue)
                xValue = TagValue.GetSetValueFive("1329", "SetValue");
        ...
        private void Chart3Bind()
        {
            ...
            try
            {
                ...
                pointX = TagValue.GetFinishedTagValueFive("F1324");//double.Parse(CalDataTable.Rows[0]["F2169"].ToString()); 
                if (pointX == GlobalVariables.BadValue)
                {
                    pointX = TagValue.GetSetValueFive("1324", "SetValue");
                }
                dygpqll = TagValue.GetShowValueFive(1324, "t/h", ""); //CalDataTable.Rows[0]["F2324"].ToString(); 
            ...
            this.label28.Text = pointY + " kPa(a)";
        ...
        AlarmReport chart1Alarm = new AlarmReport("排汽压力限制报警", 5);
        int chart1AlarmIsShow = 0;

        AlarmReport chart2Alarm = new AlarmReport("相对动应力显示报警", 5);
        int chart2AlarmIsShow = 0;

        AlarmReport chart3Alarm = new AlarmReport("背压限制报警", 5);
        int chart3AlarmIsShow = 0;

        AlarmReport ZYGCYAlarm = new AlarmReport("中压缸压差报警", 5);
        int zygcyAlarmIsShow = 0;

        AlarmReport ZYGPQWDAlarm = new AlarmReport("中压缸排气温度报警", 5);
        int zygpqwdAlarmIsShow = 0;

        AlarmReport DYGPQWDAlarm = new AlarmReport("低压缸排汽温度报警", 5);
        int dygpqwdAlarmIsShow = 0;

        AlarmReport DYGJQYLGDAlarm = new AlarmReport("低压缸进汽压力过低报警", 5);
        int dygjqylgdAlarmIsShow = 0;

        AlarmReport OPCAlarm = new AlarmReport("OPC通讯中断", 5);
        int opcAlarmIsShow = 0;
        ...
        private void Chart3Checking(double xValue, double yValue)
        {
            ...
            //斜线阶段
            else if (xValue > 191 && xValue < 556.5)
            {
                double minAlarm = xValue * 0.0629 + 7.9861;
                double maxAlarm = xValue * 0.0629 + 12.9861;
        ...
        private void OPCChecking(bool isConnect)
        {
            //if (isFirstRun)
            //    return;
            //红色报警阶段
            if (isConnect == false)
            {
                ...
                if (opcAlarmIsShow == 0)
                {
                    OPCAlarm.SetText("通讯中断！请检查网络连接", "DCS数据读取失败，请检查相关网络连接", AlarmType.AlarmOrange, 500, false, 0);
                    OPCAlarm.Show();
                    opcAlarmIsShow = 1;
                }
                else
                {
                    ...
                    OPCAlarm.SetText("通讯中断！请检查网络连接", "DCS数据读取失败，请检查相关网络连接", AlarmType.AlarmOrange, 500, false, 0);
                    //else
                    if (OPCAlarm.Visible == true)
                        OPCAlarm.Activate();
                }
            }
            else
            {
                OPCAlarm.SetText("", "", AlarmType.AlarmSafe, 0, false, 0);
            }
        }
        ...
        private void ZYGCYCheck()
        {
            ...
            cy = TagValue.GetFinishedTagValueFive("F1409");
            ...
            if (cy > 3.9)
            {
                ...
                {
                    ZYGCYAlarm.SetText("中压缸压差报警", "前后压差不得大于3.9MPa，请减少抽气量", AlarmType.AlarmWhite, 500, true, 1);
                    ZYGCYAlarm.Show();
                    zygcyAlarmIsShow = 1;
                }
                else
        ...
        private void ZYGPQWDCheck()
        {
            ...
            tempreture = TagValue.GetFinishedTagValueFive("F1407");
            ...
            if (tempreture > 378)
            {
                ...
                {
                    ZYGPQWDAlarm.SetText("中压缸排气温度报警", "中压缸排气温度大于378度，请降低再热温度", AlarmType.AlarmWhite, 500, true, 1);
                    ZYGPQWDAlarm.Show();
        ...
        private void DYGPQWDCheck()
        {
            ...
            tempreture = TagValue.GetFinishedTagValueFive("F1218");
            if (tempreture == GlobalVariables.BadValue)
                return;
        ...
        /// <summary>
        /// 低压缸进汽压力报警
        /// 暂时不用页面上的Label闪烁，因为页面上的是表压 
        /// 绝压 0.124 MPa
        /// F1169 
        /// </summary>
        private void DYGJQYLCheck()
        {
            double tempreture = 0;

            tempreture = TagValue.GetFinishedTagValueFive("F1169");
            if (tempreture == GlobalVariables.BadValue)
                return;

            if (tempreture < 0.124)
            {
                if (dygjqylgdAlarmIsShow == 0)
                {
                    DYGJQYLGDAlarm.SetText("低压缸进汽压力过低报警", "低压缸进汽压力低于0.124MPa(a)", AlarmType.AlarmWhite, 500, true, 1);
                    DYGJQYLGDAlarm.Show();
                    dygjqylgdAlarmIsShow = 1;
                }
                else
                {
                    DYGJQYLGDAlarm.SetText("低压缸进汽压力过低报警", "低压缸进汽压力低于0.124MPa(a)", AlarmType.AlarmWhite, 500, true, 1);
                    if (DYGJQYLGDAlarm.Visible == true)
                        DYGJQYLGDAlarm.Activate();
                }
                //DYGJQWDboolAlarm = true;
            }
            else
            {
                DYGJQYLGDAlarm.SetText("", "", AlarmType.AlarmSafe, 500, true, 0);
                //DYGJQWDboolAlarm = false;
            }
        ...
        bool ZYGPQWDboolAlarm = false; //中压缸排气温度
        bool DYGPQWDboolAlarm = false; //低压缸排气温度 
        //bool DYGJQWDboolAlarm = false; //低压缸进汽压力过低报警 
        public Color picColor;
        public Color parabolaColor;
        public Color brokenColor;

        Label LabelZYGYC, LabelDYGPQWD, LabelZYGPQWD;
        /// <summary>
        /// 报警提示（闪烁）
        /// </summary>
        private void AlarmHint()
        {
            ...
            if (second5 > 15)
            {
                OPCChecking(GlobalVariables.OPCState);
                second5 = 0;
            }
        ...
        //    {
        //        int A1 = (int)TagValue.GetFinishedTagValueFive("F1401");
        //        int B1 = (int)TagValue.GetFinishedTagValueFive("F1402");
        //        int C1 = (int)TagValue.GetFinishedTagValueFive("F1403");
        //        int A2 = (int)TagValue.GetFinishedTagValueFive("F1404");
        //        int B2 = (int)TagValue.GetFinishedTagValueFive("F1405");
        //        int C2 = (int)TagValue.GetFinishedTagValueFive("F1406");
        ...
        //    else
        //    {
        //        int A1 = (int)TagValue.GetFinishedTagValueFive("F2401");
        //        int B1 = (int)TagValue.GetFinishedTagValueFive("F2402");
        //        int C1 = (int)TagValue.GetFinishedTagValueFive("F2403");
        //        int A2 = (int)TagValue.GetFinishedTagValueFive("F2404");
        //        int B2 = (int)TagValue.GetFinishedTagValueFive("F2405");
        //        int C2 = (int)TagValue.GetFinishedTagValueFive("F2406");
        ...
        {
            FrmShowHis myfrmshouHis = new FrmShowHis(5);
            try
            {
                LabelTag labelTag = new LabelTag();
                labelTag.TagID = 1186;
                labelTag.TagDesc = "实时测量发电煤耗";
                Label lblNew = new Label();
                lblNew.Tag = labelTag;
                label = lblNew;

                myfrmshouHis.fczMain = this;
        ...
        private static bool CheckTags(DataTable dttags)
        {
            ...
            if (dtCurrentUnitTag == null || dtCurrentUnitTag.Rows.Count <= 0)
            {
                ...
                dtCurrentUnitTag = GlobalVariables.dtTagsSetFive.DefaultView.ToTable();
        ...
        void Label_DoubleClick(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                string str = TagValue.GetSetValueFive(labelId, "adjustValue") + "";
        ...
        private void contextMenuStrip_Label_Opening(object sender, CancelEventArgs e)
        {
